...
--  (hawpplpfychvjahaywum). Paste the whole file into the Supabase
--  SQL editor and hit Run. Safe to re-run — every statement is
--  idempotent.
--
--  Part 1  Onboarding schema (additive)  <- from migration_onboarding.sql
--  Part 2  Image storage buckets         <- from storage.sql
--
--  NOTE: do NOT run supabase/schema.sql on this project. It is the
--  reference schema for a brand-new project and uses uuid ids /
--  user_id keys, which do not match this project's text ids /
--  profile_id keys.
-- =====================================================================
