...
export default function PostDetailsView({ post, interestedCount, userReaction, onBack, onReact }: PostDetailsViewProps) {
  ...
  return (
    <div className="space-y-5 rounded-[28px] border border-[#00ffc8]/20 bg-[#041a14]/90 p-4 shadow-[0_20px_50px_rgba(0,0,0,0.8)] md:p-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.24em] text-emerald-100/80"
      >
        <ArrowLeft className="w-3.5 h-3.5" />
        <span>Back to Posts</span>
      </button>

      <div className="overflow-hidden rounded-[28px] border border-[#00ffc8]/20">
        <div className="relative h-72">
          ...
          <div className="absolute bottom-4 left-4 right-4">
            ...
            <div className="mt-2 flex items-center gap-1.5 text-[11px] font-black uppercase tracking-[0.24em] text-[#00ffc8]">
        ...
        <div className="grid gap-3 p-4 md:grid-cols-2">
          <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
            <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.24em] text-[#00ffc8]">
              <CalendarClock className="w-3.5 h-3.5" />
              <span>Run Details</span>
            </div>
            <div className="mt-3 space-y-2 text-xs text-emerald-100/85">
              ...
              <div className="flex items-center justify-between"><span>Estimated Time</span><span className="font-bold text-white">{trail?.estimatedTimeMin ?? "—"} min</span></div>
            </div>
          </div>

          <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
            <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.24em] text-[#00e5ff]">
              <Users className="w-3.5 h-3.5" />
              <span>Community Plan</span>
            </div>
            <p className="mt-3 text-xs leading-relaxed text-emerald-100/80">{post.description}</p>
            <div className="mt-4 flex flex-wrap gap-2">
              <ReactionButtons userReaction={userReaction} interestedCount={interestedCount} onReact={onReact} />
              <button type="button" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.24em] text-emerald-100/80">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
          <div className="mb-3 text-[10px] font-black uppercase tracking-[0.24em] text-[#00ffc8]">Interactive Map</div>
          <div className="rounded-2xl border border-[#00ffc8]/20 bg-cover bg-center p-3" style={{ backgroundImage: `url('${trail?.image || "https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=1200&q=80"}')` }}>
            <div className="rounded-2xl border border-white/10 bg-[#020b08]/70 p-4 text-xs text-emerald-100/85">
              <div className="flex items-center gap-2 text-[#00ffc8] font-black uppercase tracking-[0.2em]"><MapPin className="w-3.5 h-3.5" /> <span>{trail?.location || "Map preview"}</span></div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-white/10 bg-black/25 p-4 text-xs text-emerald-100/80">
            <div className="text-[10px] font-black uppercase tracking-[0.24em] text-[#00e5ff]">Future placeholders</div>
